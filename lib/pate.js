...
exports.parse = function parse(options, callback) {
    ...
    try {
        ...
        // XPath match
        new_tpl = new_tpl.replace(/\{\{(.*?)\}\}/g, function (m, t) {
            return xpathFn(m, t, rootEl, options.ns);
        });

        // Eval match
        new_tpl = new_tpl.replace(/\[\[(.*?)\]\]/g, function (m, t) {
            return evalFn(m, t, options.format_lib);
        });
 ...
 */
var xpathFn = function xpathFn(match, token, rootEl, ns) {
 ...
 */
var evalFn = function evalFn(match, token, format_lib) {
